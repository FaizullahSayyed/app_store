...
class AppStore extends Component {
  state = {activeTab: tabsList[0].tabId, searchText: ''}

  getAppsList = () => {
    const {activeTab, searchText} = this.state
    const filteredData = appsList.filter(app => app.category === activeTab)
    if (searchText) {
      const searchResult = filteredData.filter(app =>
  ...
  render() {
    const filteredAppsList = this.getAppsList()
